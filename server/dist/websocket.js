...
function getChunkKey(chunkX, chunkY, zoom) {
    return `chunk:${zoom}:${chunkX}:${chunkY}`;
}
function getVisibleChunks(bounds, zoom) {
    if (zoom < 10)
        return [];
...
async function mockVerifyToken(token) {
...
async function mockGetPixelsInBounds(bounds, zoom) {
    // Temporaire - retourne un tableau vide
    return [];
}
async function mockPlacePixel(lat, lng, color, userId) {
...
async function mockGetUserStats(userId) {
    return {
        pixelsPlaced: 42,
        pixelsCurrent: 25,
        canPlacePixel: true
...
app.prepare().then(() => {
    ...
    io.on('connection', async (socket) => {
        ...
        socket.on('view_changed', async (data) => {
            const user = connectedUsers.get(socket.id);
            if (!user)
                return;
            try {
                const { bounds, zoom } = data;
                const visibleChunks = getVisibleChunks(bounds, zoom);
                const newChunkKeys = new Set(visibleChunks.map(chunk => getChunkKey(chunk.chunkX, chunk.chunkY, zoom)));
                // Quitter les anciens chunks
                user.currentChunks.forEach((chunkKey) => {
                ...
                newChunkKeys.forEach((chunkKey) => {
                    if (!user.currentChunks.has(chunkKey)) {
                        ...
                        if (!chunkUsers.has(chunkKey)) {
                            chunkUsers.set(chunkKey, new Set());
                        }
